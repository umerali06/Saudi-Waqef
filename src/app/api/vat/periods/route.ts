...
import { vatPeriodSchema } from "@/lib/validators/company";
...
export async function POST(request: Request) {
  ...
  const parsed = vatPeriodSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const periodId = await createVatPeriod(parsed.data);
  await recordAuditEvent({
    companyId: parsed.data.companyId,
    userId: user.id,
    userEmail: user.email ?? undefined,
    action: "vat_period.create",
    entity: "vat_period",
    entityId: periodId,
    metadata: {
