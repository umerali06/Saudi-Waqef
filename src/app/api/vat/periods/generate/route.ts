...
import { listVatPeriods, createVatPeriod } from "@/lib/data/vat-periods";
import { vatPeriodGenerateSchema } from "@/lib/validators/company";
import {
  buildMonthlyPeriods,
...
export async function POST(request: Request) {
  ...
  const parsed = vatPeriodGenerateSchema.safeParse(body);
  if (!parsed.success) {
  ...
  const existing = await listVatPeriods(parsed.data.companyId);
  ...
  if (hasOverlap) {
    return NextResponse.json(
      { error: "VAT period overlaps an existing period" },
      { status: 409 }
    );
  }
  ...
  for (const draft of drafts) {
    const periodId = await createVatPeriod({
