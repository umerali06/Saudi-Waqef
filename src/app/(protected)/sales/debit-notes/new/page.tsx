...
export default function NewDebitNotePage() {
  ...
  const handleSubmit = (status: "draft" | "issued") => {
    if (!activeCompanyId || !invoiceId) {
      setErrorKey("debitNote.selectInvoiceError");
      return;
    }
    ...
    startTransition(async () => {
      ...
      router.push(`/sales/debit-notes/${data.debitNoteId}`);
    });
  };
  ...
  return (
    <section className="space-y-6 page-shell">
      <div>
        <h1 className="text-2xl font-semibold page-title">{t("debitNote.createTitle")}</h1>
        <p className="text-sm text-muted page-subtitle">{t("debitNote.createSubtitle")}</p>
      </div>

      <div className="app-card p-6 card-modern">
        <h2 className="text-lg font-semibold">{t("debitNote.selectInvoice")}</h2>
        <div className="mt-4 grid gap-4 md:grid-cols-3">
          <label className={`text-sm ${alignClass}`}>
            <span className="mb-1 block text-xs text-muted">{t("debitNote.invoice")}</span>
            <select
              className="w-full rounded-2xl border border-border bg-surface px-3 py-2 text-sm"
              value={invoiceId}
              onChange={(event) => setInvoiceId(event.target.value)}
            >
              <option value="">{t("debitNote.selectInvoicePlaceholder")}</option>
              {invoices.map((entry) => (
          ...
          <label className={`text-sm ${alignClass}`}>
            <span className="mb-1 block text-xs text-muted">{t("debitNote.issueDate")}</span>
            <input
              type="date"
              className="w-full rounded-2xl border border-border bg-surface px-3 py-2 text-sm"
              value={issueDate}
