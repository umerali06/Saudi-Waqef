...
type DebitNoteStatus = "draft" | "issued" | "canceled";
...
const STATUS_STYLES: Record<DebitNoteStatus, string> = {
  draft: "bg-amber-100 text-amber-800",
  issued: "bg-blue-100 text-blue-800",
  canceled: "bg-rose-100 text-rose-800",
};

const mapDebitNoteError = (error?: string) => {
  switch (error) {
    case "Credit exceeds balance":
      return "debitNote.exceedsBalance";
    case "Debit note is locked":
      return "debitNote.locked";
    case "Invalid invoice":
      return "debitNote.invalidInvoice";
    case "Invoice is canceled":
      return "debitNote.invoiceCanceled";
    case "Invalid item":
      return "debitNote.invalidItem";
    case "Invalid unit":
      return "debitNote.invalidUnit";
    case "Missing receivable account":
      return "debitNote.missingReceivableAccount";
    case "Missing sales account":
      return "debitNote.missingSalesAccount";
...
export default function DebitNoteDetailPage() {
  const params = useParams<{ debitNoteId: string }>();
  const debitNoteId = params.debitNoteId;
  ...
  const loadNote = useCallback(async () => {
    ...
    const nextNote = data.debitNote as DebitNote | undefined;
    if (!nextNote) {
      setNote(null);
      setLoadingNote(false);
      return;
  ...
  const handleSave = () => {
    ...
    if (payloadLines.length === 0) {
      setErrorKey("debitNote.linesRequired");
      return;
    }
  ...
  const handleIssue = () => {
    ...
    startTransition(async () => {
      setErrorKey(null);
      setNoticeKey(null);
      const response = await fetch(`/api/debit-notes/${debitNoteId}/issue`, {
        method: "POST",
      });
      const data = await response.json().catch(() => ({}));
      if (!response.ok) {
  ...
  const handleCancel = () => {
    ...
    const confirmed = window.confirm(t("debitNote.cancelConfirm"));
    if (!confirmed) {
      return;
    }
    startTransition(async () => {
  ...
  const handleSend = () => {
    ...
    startTransition(async () => {
      setErrorKey(null);
      setNoticeKey(null);
      const response = await fetch(`/api/debit-notes/${debitNoteId}/send`, {
        method: "POST",
      });
      if (!response.ok) {
        setErrorKey("error.saveFailed");
  ...
  return (
    <section className="space-y-6 page-shell">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs text-muted">{t("debitNote.detailsTitle")}</p>
          <div className="flex flex-wrap items-center gap-2">
            <h1 className="text-2xl font-semibold page-title">{note.debitNumber}</h1>
            <span
          ...
          <p className="text-sm text-muted page-subtitle">{note.customerName}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Link
      ...
      <div className="app-card p-6 card-modern">
        <div className="grid gap-4 md:grid-cols-3">
          <div>
            <p className="text-xs text-muted">{t("debitNote.invoice")}</p>
            <Link
          ...
          <div>
            <p className="text-xs text-muted">{t("debitNote.issueDate")}</p>
            {isDraft ? (
              <input
                type="date"
          ...
          <div className={alignClass}>
            <p className="text-xs text-muted">{t("debitNote.availableBalance")}</p>
        ...
        {isDraft ? (
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <label className={`text-sm ${alignClass}`}>
              <span className="mb-1 block text-xs text-muted">{t("debitNote.reason")}</span>
              <input
          ...
          <>
            {note.reason ? (
              <p className="mt-4 text-sm text-muted page-subtitle">
                {t("debitNote.reason")}: {note.reason}
              </p>
            ) : null}
            {note.notes ? (
      ...
      <div className="app-card p-6 card-modern">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h2 className="text-lg font-semibold">{t("debitNote.linesTitle")}</h2>
          <span className="text-xs text-muted">{lines.length}</span>
        </div>
        {lines.length === 0 ? (
          <p className="mt-4 text-sm text-muted page-subtitle">{t("debitNote.linesEmpty")}</p>
        ) : isDraft ? (
            ...
            <table className="min-w-full text-sm table-modern">
              <thead className="bg-surface-muted text-muted thead-modern">
                <tr>
                  ...
                  <th className={`px-3 py-2 ${alignClass}`}>{t("debitNote.restock")}</th>
                </tr>
              </thead>
