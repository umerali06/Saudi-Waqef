export type MudadResultStatus = "accepted" | "rejected" | "submitted";

export type NormalizedMudadResult = {
  employeeId: string;
  status: MudadResultStatus;
...
const normalizeStatus = (candidate: unknown): MudadResultStatus => {
...
export const normalizeMudadResults = (payload: unknown): NormalizedMudadResult[] => {
  ...
  const normalized: NormalizedMudadResult[] = [];
